...
* Date : Feb 13, 2023.
* Description : Example 2 - Increasing the number of photons and modes
...
int main(int argc, char* argv[]) {
    ...
    // Loop for different number of modes
    for (int n = 4; n < max_input + 1; n += 2) {
        ...
        //nn.push_back(n);
        vector<int> input = a;
        int modes = input.size();
        vector<int> output = input;

        vector<vector<vector<complex<double>>>> beam_splits = bs_parameters(true, n, depth);
        ...
        cout << "Probability_Glynn = " << probability_Glynn(unitary_st, input, output, depth) << endl;
