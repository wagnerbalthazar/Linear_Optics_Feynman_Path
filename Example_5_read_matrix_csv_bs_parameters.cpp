...
* Date : Feb 13, 2023.
* Description : Example 5 - read two csv files with beam splitter paramenters
...
vector<vector<double>> zeros_matrix_double(int n, int m) {
    /// <summary>
...
double probability_Feynman(vector<int> inp, vector<int> out, int dep, vector<vector<vector<complex <double>>>> bss)
{
    ...
    if (sum_inp != sum_out || inp.size() != out.size() || inp.size() % 2 != 0 || out.size() % 2 != 0) {
        ...
        if (dep % 2 != 0) {
            ...
            vector<vector<int>> general_matrix_loop_2_l = light_cone(inp, out, dep);
            vector<vector<int>> general_matrix_loop_2_r = light_cone(inp, out, dep);*/
            //vector<vector<int>> general_matrix_loop_2 = light_cone_left_right(inp, out, dep);
            vector<vector<int>> general_matrix_loop_2 = light_cone(inp, out, dep);
...
double probability_Glynn(vector<vector<complex<double>>> matrix, vector<int> inp, vector<int> out, int dep) {
    ...
    /// </summary>
    /// <param name="mat"><> /Matrix U that represents the interferometers given by clement's design.
    /// <param name="inp"><> Vector with input states (number of photons in each port).
    /// <param name="out"></param> Vector with output states (number of photons in each port).
    /// <param name="dep"></param> The depth of the circuit.
    /// <returns></returns> A double with the probability.
...
vector<vector<double>> read_matrix_csv(const string& fileName)
{
...
int main(int argc, char* argv[]) {
    // Initial conditions
    const vector<int> input = { 2,0,2,0,0,2 };
    const vector<int> output = { 1,1,1,1,1,1 };

    const int sum_inp = sum_vector(input, 0, input.size());
    const int modes = input.size();
    int depth = 5;
    int number_of_row_parameters = 10;

    // Reading the beam splitters parameters
    vector<vector<double>> t_1 = read_matrix_csv("thetas.csv");
    vector<vector<double>> t_2 = read_matrix_csv("phis.csv");

    // Run the code for all interferometers defined by the parameters uploaded in csv files.

    for (int i = 0; i < t_1.size(); ++i) {
        vector<double> t_11;
        vector<double> t_22;
        for (int j = 0; j < t_1[0].size(); ++j) {

            t_11.push_back(t_1[i][j]);
            t_22.push_back(t_2[i][j]);
        }
        vector<vector<vector<complex<double>>>> beam_splits = bs_parameters_2(t_11, t_22, modes, depth);
        cout << "Probability_Feynman = " << probability_Feynman(input, output, depth, beam_splits) << endl;
        ...
        cout << "probability_ouputs_Ryser = " << probability_Ryser(unitary_st, input, output, depth) << endl;
        cout << "probability_ouputs_Glynn = " << probability_Glynn(unitary_st, input, output, depth) << endl;
    }
