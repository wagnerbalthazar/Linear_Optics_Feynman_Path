...
* Date : Feb 13, 2023.
* Description : Additional code - Ryser's Formula.
...
experimental::generator<vector<int>> odo_incr(vector<int> vector_odo) {

    /// <summary>
    /// This function finds all possible combinations of photons occupations in the green waveguides. The code runs from 0 to maximum allowed value for the 
    /// photon occupation in green waveguides.
    /// </summary>
    /// <param name="vector_odo"></param> A vector with all maximum values of green waveguides.
    /// <returns></returns> A bunch of vectors (one by one) that contain all possible combinations of photon occupations in the green waveguides.
    ...
    vector<int> max_ranges = vector_odo;
    vector<int> initial(max_ranges.size(), 0);
    int pos = max_ranges.size() - 1;
    ...
    while (greens != max_ranges) {
        if (incr == 0) {
            greens = initial;
            incr += 1;
            co_yield greens;
        }
        else if (greens[pos] == max_ranges[pos]) {
            greens[pos] = 0;
            pos -= 1;
            continue;
        }
...
vector<vector<complex<double>>> Unitary_ST(vector<vector<complex<double>>> mat, vector<int> inp, vector<int> out) {
    ...
    if (sum_inp != sum_out || inp.size() != out.size() || inp.size() % 2 != 0 || out.size() % 2 != 0) {
        throw runtime_error("The sum of the elements of vectors input and output must have the same number of photos, the same length and an even number os elements.");
...
int main(int argc, char* argv[]) {
    ...
    cout << "Probabilty Ryser = " << probability_Ryser(unitary_st, input, output, depth) << endl;
